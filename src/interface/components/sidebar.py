...
def render_sidebar():
    ...
    st.sidebar.markdown(
        ...
        <div style="text-align: center; padding: 10px 0;">
            <h2 style="color: #008069; margin: 0; font-size: 20px;">🏥 Hospital AI Assistant</h2>
            <p style="color: #667781; font-size: 13px; margin: 5px 0 15px 0;">Hospital Appointment Booking System</p>
        </div>
        """, 
    ...
    st.sidebar.markdown("---")
    st.sidebar.markdown("### 🏢 Hospital Organization")
    
    tenant_options = {
        "Gleneagles Hospitals": "gleneagles",
        "Apollo Hospitals":     "apollo",
        "Aster Healthcare":     "aster",
        "Fortis Healthcare":    "fortis",
    }
    
    if "tenant_id" not in st.session_state:
        st.session_state.tenant_id = "gleneagles"
    ...
    if st.sidebar.button("🔄 Reset Conversation", use_container_width=True):
        with st.spinner("Clearing database history..."):
            success = reset_session_on_backend(customer_phone, tenant_id=st.session_state.tenant_id)
            if success:
