...
def parse_and_render_invoice(text: str) -> bool:
    ...
    if "Download Invoice PDF" in text:
        ...
        if url_match:
            ...
            biz_name = biz_match.group(1) if biz_match else os.getenv("DEFAULT_BIZ_NAME", "Hospital Appointment Booking System")
            
            # Extract delivery estimation line if present (starts with 🚚)
            delivery_match = re.search(r'(🚚[^\n]+)', text)
            delivery_msg = delivery_match.group(1) if delivery_match else ""
            ...
            st.markdown(
                ...
                <div class="invoice-card" style="font-family: inherit;">
                    ...
                    {f'<div style="margin-bottom: 12px; font-size: 13px; color: #008069; font-weight: 500; background-color: #e8f5e9; padding: 6px 10px; border-radius: 4px; display: inline-block;">{delivery_msg}</div>' if delivery_msg else ''}
